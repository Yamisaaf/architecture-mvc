...
// class permet de gerer le cache
Class Cache {
    ...
    public function write($cachename, $content = null) {
        ...
        $this->path = BASE . Base::get()->config['Cache']['dossier'];
        $this->chemin = BASE . Base::get()->config['Cache']['dossier'] . DS . $cachename . Base::get()->config['Cache']['extention'];
    ...
    public function read($cachename) {
        ...
        if ($this->time > Base::get()->config['Cache']['time']) {
            if (file_exists($this->chemin)) {
